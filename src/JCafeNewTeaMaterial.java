...
class JCafeNewTeaMaterial extends JDialog implements ActionListener{
	JTable table;
	...
	File file = new File("JCafeData/InitData/InitMaterialData/", "tea_material");
	File fileTemp = new File("JCafeData/InitData/InitMaterialData/", "tea_temp");
	File fileM = new File("JCafeData/InitData/InitMaterialData/", "te_temp");
	int sel;
	String mName;
	
	JCafeNewTeaMaterial(int sel, String mName){
		...
		try{
			...
			for(int i = 0; i < str.length; i++) {
				...
				if(strs[0].equals(mName)) {
					...
					bw.write(str[i]);
					//tea_material에서 같은 메뉴가있으면 그 메뉴줄 빼고 '가상파일'에 '나머지'를 저장한다.
					bw.newLine();
				}
			}
			...
			for(int i = 0; i < table.getRowCount(); i++){
				if(table.getValueAt(i, 2) != null){
					...
					bw.write(ingre + "/" + gram + "/" + cnt + "/");	//예) 아메/우유/g/14
				}
			}
			bw.newLine();
			br.close();
